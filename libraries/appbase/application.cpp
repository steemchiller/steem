...
void application::set_program_options()
{
   ...
   app_cfg_opts.add_options()
      (
         "plugin",
         bpo::value< vector<string> >()->composing()->default_value( default_plugins, plugins_ss.str() ),
   ...
   app_cli_opts.add_options()
      (
         ...
         "data-dir,d",
         bpo::value< bfs::path >(),
         data_dir_ss.str().c_str()
      )(
         "config,c",
         bpo::value< bfs::path >()->default_value( "config.ini" ),
...
#endif
         data_dir = parent != nullptr ? std::string( parent ) : bfs::current_path();
...
abstract_plugin* application::find_plugin( const string& name ) const
{
   ...
   return itr == plugins.end() ? nullptr : itr->second.get();
}
